...
export default function NewEntry() {
  ...
  const { kind, from } = useLocalSearchParams<{ kind?: 'transfer' | 'adjustment'; from?: string }>();
  const entries = useEntries();
  const template = from ? entries.find((e) => e.id === from) : undefined;
  const addEntry = useStore((s) => s.addEntry);
  const rules = useWriteRules();
  const allowed = [...(rules.canWriteExpense ? ['expense' as const] : []), ...(rules.canWriteTransfer ? ['transfer' as const] : []), ...(rules.canWriteExpense ? ['adjustment' as const] : [])];
  if (allowed.length === 0) return <Screen><Body muted>{t('trip.readOnly', { status: t(`trip.status.${rules.status}`) })}</Body></Screen>;
  const only = kind && allowed.includes(kind) ? [kind] : allowed;
  return (
    <>
      <Stack.Screen options={{ title: only.length === 1 && only[0] === 'transfer' ? t('entry.newPayment') : only.length === 1 && only[0] === 'adjustment' ? t('entry.newAdjustment') : t('entry.new') }} />
