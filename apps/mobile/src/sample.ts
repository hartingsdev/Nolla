...
import { type Participant, type TripMeta } from './store';

/** The first rows of the group's real spreadsheet, so screens have something to show. */
export function sampleTrip(): { trip: TripMeta; participants: Participant[]; entries: WireEntry[] } {
  ...
  const rows: [string, string, string, string, ParticipantId[]][] = [
    ['Miete 1/2', '465.35', 'Robert', '2025-11-18', all],
    ['Reinigungskosten', '75.00', 'Robert', '2026-03-01', all],
    ['Einkauf Krefeld', '55.18', 'Marc', '2026-03-01', all],
    ['Lidl Samstag', '75.17', 'Marc', '2026-03-01', all],
    ['Tanken Samstag', '72.53', 'Yannik', '2026-03-01', all],
    ['Maut Tobias', '4.00', 'Tobias', '2026-03-01', all],
    ['Rulantica', '168.00', 'Marc', '2026-03-02', [id('Yannik'), id('Max'), id('Robert'), id('Tobias')]],
    ['Pfandsammlung', '-8.75', 'Marc', '2026-03-04', all],
  ];
  let k = 0;
  const entries = rows.map(([description, amt, payer, date, among]) => {
    const amount = moneyFromString(amt, EUR);
    const eid = `sample-${String(k++)}`;
    return entryToWire({
      id: eid, type: 'expense', description, amount, date: localDate(date),
  ...
  entries.push(entryToWire({
    id: 'sample-x', type: 'expense', description: 'Essen Saarbrücken', amount: s, date: localDate('2026-03-03'),
